...
class PlayerAdapter(val context: Context, val withPower:Boolean) : RecyclerView.Adapter<PlayerAdapter.PlayerHolder>() {

    val players : List<Player> = LocalBase.instance.players

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PlayerHolder {
        if(withPower) {
            return PlayerHolder(LayoutInflater.from(context).inflate(R.layout.one_player_with_power,parent,false), withPower)
        }else{
            return PlayerHolder(LayoutInflater.from(context).inflate(R.layout.one_player,parent,false), withPower)
        }
    }
    ...
    class PlayerHolder(itemView: View, withPower: Boolean) : RecyclerView.ViewHolder(itemView) {
